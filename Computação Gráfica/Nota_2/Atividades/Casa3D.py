...
Vao_Objetocarro = None
obj_indicescarro = None
obj_buffercarro = None
obj_texturacarro = None
...
def inicializaObjeto():
    ...
    obj_indices, obj_buffer = ObjLoader.load_model("meshes/newhouse.obj")
    ...
    load_texture("texturas/Telhado.jpg", obj_textura)

def inicializaObjetocarro():
    global Vao_Objetocarro, obj_buffercarro, obj_indicescarro, obj_texturacarro
    obj_indicescarro, obj_buffercarro = ObjLoader.load_model("meshes/carro2.obj")

    #Vao do objeto
    Vao_Objetocarro = glGenVertexArrays(1)
    glBindVertexArray(Vao_Objetocarro)

    #VBO do objeto. Ao invés de termos 1 VBO para cada informação (vértices, texturas, normais, etc.), este exemplo utiliza
    #1 único VBO com todas as informações dentro
    bvbo = glGenBuffers(1)
    glBindBuffer(GL_ARRAY_BUFFER, bvbo)
    glBufferData(GL_ARRAY_BUFFER, obj_buffercarro.nbytes, obj_buffercarro, GL_STATIC_DRAW)
    # vertices
    glEnableVertexAttribArray(0)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, obj_buffercarro.itemsize * 8, ctypes.c_void_p(0))
    # texturas
    glEnableVertexAttribArray(1)
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, obj_buffercarro.itemsize * 8, ctypes.c_void_p(12))
    # normais
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, obj_buffercarro.itemsize * 8, ctypes.c_void_p(20))
    glEnableVertexAttribArray(2)

    obj_texturacarro = glGenTextures(1)
    load_texture("texturas/cube.jpg", obj_texturacarro)
...
def inicializaRenderizacao():
    ...
    while not glfw.window_should_close(Window):
        ...
        glBindVertexArray(Vao_Objetocarro) #ativamos o objeto que queremos renderizar
        glBindTexture(GL_TEXTURE_2D,obj_texturacarro)
        transformacaoGenerica(2,0.8,-3,1,1,1,0,90,0)
        glDrawArrays(GL_TRIANGLES, 0, len(obj_indicescarro))
